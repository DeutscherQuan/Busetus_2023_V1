...
def handler(signum, frame):
	print('Procedure is taking too long',signum)
...
if __name__ == '__main__':
	...
	pressu = str(round(data[0], 2))	
	temp = str(round(data[1], 2))
	humi = str(round(data[2], 2))	
	while True:
		#if (schedule.every().day.at("21:43")):
		#if internet_on():
		#try:
			...
			try:
				...
				if (today.minute % 1 == 0):
					database = firebase.database()
					database.child("Environment Sensor Storage").child("BME Storage").child("Pressure").push(pressu)
				#time.sleep(0.5)     
